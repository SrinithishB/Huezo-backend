...
class OrderCreateView(APIView):
    ...
    def post(self,request,order_type):
        ...
        if not serializer_class:
            return Response({"error":"Invalid order type. Use wl, pl, or fabrics."},status=status.HTTP_400_BAD_REQUEST)

        images_data = request.FILES.getlist("images")
        serializer = serializer_class(data={**request.data,"images":images_data},context={"request":request})

        if serializer.is_valid():
            order = serializer.save()
            response = OrderDetailSerializer(order,context={"request":request})
            return Response({"message":"Order placed successfully.","data":response.data},status=status.HTTP_201_CREATED)
...
class OrderListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        qs = Order.objects.select_related("customer_user","white_label_catalogue","fabric_catalogue")

        if user.role == "customer": return qs.filter(customer_user=user)
        return qs.all()
...
class OrderDetailView(APIView):
    ...
    def get_object(self,id,user):
        try:
            ...
            if user.role == "customer": return qs.get(id=id,customer_user=user)
            return qs.get(id=id)
